/**
 * customer-master.js - 共通顧客マスタ管理モジュール
 * 弥生販売の得意先リストから顧客情報を取得
 * 小売・卸売両方で使用
 */

const STORAGE_KEY = 'customerMaster';

// ドメインマッピング（卸売用、動的に設定可能）
let domainToNameMapping = {};
...
/**
 * ドメインマッピングを設定（卸売モジュールから呼び出し）
 * @param {Object} mapping - { domain: japaneseName } のマッピング
 */
export function setDomainMapping(mapping) {
    domainToNameMapping = mapping || {};
}
